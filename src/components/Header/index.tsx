...
export const Header: React.FC<ThemeProps> = ({ theme }) => {
  ...
  //   return () => {
  //     window.removeEventListener('scroll', changeValueScroll);
  //   };
  // }, []);
  React.useEffect(() => {
    if (burger) {
  ...
  const clickTheme = () => {
    ...
    dispatch(setMode(theme === 'light' ? 'dark' : 'light'));
  };
  ...
  return (
    <header
      className={classNames(s.header, {
        [s.header_dark]: theme === 'dark',
        header_scroll: scroll === true,
      })}>
      <div className="container">
        <div className={s.header_row}>
          ...
          <div
            className={classNames(s.header_burger, {
              [s.active]: burger === true,
            })}
            onClick={handleBurger}>
            <span></span>
          </div>
          <nav
            className={classNames(s.header_menu, {
              [s.active]: burger === true,
            })}>
            <ul>
              <li>
                <Link
                  to="/"
                  className={classNames(s.header_menu__link, {
                    [s.active]: 1 === 1,
                  })}
                  data-testid="main-link">
                  {/* <Link to="/" className={s.header_menu__link} data-testid="main-link"> */}
                  Главная
                </Link>
              </li>
              <li>
                <Link to="/about" className={s.header_menu__link} data-testid="about-link">
                  Обо мне
                </Link>
              </li>
            </ul>
            <div className={s.header_mode}>
              {theme === 'light' ? (
                <svg
                  onClick={clickTheme}
                  className={s.header_mode_sunIcon}
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                ...
                <svg
                  onClick={clickTheme}
                  className={s.header_mode_moonIcon}
                  width="19"
                  height="19"
                  viewBox="0 0 19 19"
                  fill="none"
          ...
          {isAuth && (
            <div className={s.header_admin}>
              <Link to="/add-post" className={s.header_admin_write}>
